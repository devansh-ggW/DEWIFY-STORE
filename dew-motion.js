...
(function(){
  ...
  function burst(x,y,count=10,scale=1){
    ...
    window.setTimeout(()=>wrap.remove(),760);
  }
  ...
  function tapBurst(x,y){
    ...
    for(let i=0;i<count;i++){
      ...
      const a=(Math.PI*2*i/count);const d=13+Math.random()*10;
  ...
  function getCardImage(card){
    return card?.querySelector(".product-card-image, .product-visual img")||null;
  }

  function flyToBag(card){
    if(reduceMotion||!card)return;
    const source=getCardImage(card);const bag=qs(".bag-button");
    if(!source||!bag)return;
    const a=source.getBoundingClientRect();const b=bag.getBoundingClientRect();
    if(a.width<4||a.height<4)return;
    const node=source.cloneNode(true);
    node.classList.add("dew-flight");
    ...
    void node.offsetWidth;node.classList.add("dew-flight-go");
    window.setTimeout(()=>{ burst(tx,ty,12,1.05); },700);
    window.setTimeout(()=>node.remove(),1030);
  }

  function animateFilter(grid){
    if(!grid||reduceMotion)return;
    grid.classList.remove("dew-filtering");
    void grid.offsetWidth;grid.classList.add("dew-filtering");
    window.setTimeout(()=>{
      qsa(".product-card",grid).forEach((card,i)=>{
        card.classList.remove("dew-filter-in");
        card.style.setProperty("--filter-delay",`${Math.min(i*42,260)}ms`);
        void card.offsetWidth;card.classList.add("dew-filter-in");
      });
      grid.classList.remove("dew-filtering");
    },90);
  }
  ...
  function setupCardParallax(){
    ...
    document.addEventListener("pointermove",e=>{
      ...
      raf(()=>{
        ...
        const r=active.getBoundingClientRect();const px=(lastX-r.left)/r.width-.5;const py=(lastY-r.top)/r.height-.5;
        active.style.setProperty("--image-shift-x",`${(-px*7).toFixed(2)}px`);active.style.setProperty("--image-shift-y",`${(-py*7).toFixed(2)}px`);
      });
    },{passive:true});
  }

  function setupScrollReveal(){
    const targets=qsa("main section, .section-heading, .filters, .product-card, footer");
    ...
    targets.forEach((el,i)=>{
      ...
      el.classList.add("dew-scroll-reveal");el.style.transitionDelay=`${Math.min(i*22,140)}ms`;io.observe(el);
    });
  }
  ...
  function setup(){
    ...
    setupCardParallax();setupScrollReveal();setupHeader();
    ...
    if(grid&&!reduceMotion){
      const observer=new MutationObserver(()=>animateFilter(grid));
      observer.observe(grid,{childList:true});
      let initialized=false;
      const initialObserver=new MutationObserver(()=>{if(!initialized){initialized=true;animateFilter(grid);}});
      initialObserver.observe(grid,{childList:true});
      window.setTimeout(()=>initialObserver.disconnect(),1500);
    }

    // Soft press feedback on product cards without fighting the click handler.
    document.addEventListener("pointerdown",e=>{
      const card=e.target.closest?.(".product-card");
      if(card&&!reduceMotion)card.style.setProperty("--press-scale",".995");
    },{passive:true});
